...
class DemoNode(Node):
    ...
    def sendcmd(self):
        ...
        if (self.t <= GO_TO_START_T) and self.init:
          q, qDot = spline(t, GO_TO_START_T, np.array(self.idleJoinPos), 
                        self.idlePosition, np.zeros((3)), np.zeros((3)))    
          e, ed = spline(t, GO_TO_START_T - 1.0, np.zeros((3)), 
        ...
        # time sendcmd is called the if statement will not be entered. 
        elif self.start and self.touch:
            print('x:')
            x = input()
            print('y:')
            y = input()          
            ...
            userX = x
            userY = y
            userPosition = np.array([userX, userY, 0]) 
        ...
        if (self.t <= GO_TO_T and self.touch):
            ...
            p, v = spline(self.t, GO_TO_T, np.array(self.taskPos0), 
                          np.array(userPosition), np.zeros((3)), np.zeros((3)))
        ...
        (p, r, Jv, Jw) = self.chain1.fkin(self.q)
