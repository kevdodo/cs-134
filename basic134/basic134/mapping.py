...
class DemoNode(Node):
    ...
    def __init__(self, name):
        ...
        self.pubpose = self.create_publisher(Pose, "/pose", 10)
...
#ros2 topic pub -1 /point geometry_msgs/msg/Point "{x: 0.35, y: 0.5, z: 0.2}"
    ...
    def get_contours(self, frame):
        ...
        if len(contours) > 0:
            ...
            trans_mat = self.get_transformation(frame, 0.0, 0.5, annotateImage=True)
    ...
    # 
                    ...
                    if isbox:
                        box_world = self.transform_box(frame, trans_mat, box, annotateImage=True)
                        [[x, y]] = self.transform_box(frame, trans_mat, [[center_x, center_y]], annotateImage=True)
                        ...
                        point_pose = Point()
                        point_pose.x = float(x)
                        point_pose.y = float(y)
                        point_pose.z = 0.0
                        quat_pose = Quaternion()
                        quat_pose.x = 0.0
                        quat_pose.y = 0.0
                        quat_pose.z = np.sin(angle / 2)
                        quat_pose.w = np.cos(angle / 2)
                        pose = Pose()
                        pose.position = point_pose
                        pose.orientation = quat_pose
    ...
    # Pixel Conversion
    def get_transformation(self, image, x0, y0, annotateImage=False):
        '''
    ...
    def transform_box(self, frame, M, box, annotateImage=True):
        ...
        for u, v in box:
            ...
            if annotateImage:
                ...
                cv2.putText(frame, s, (u-80, v-8), cv2.FONT_HERSHEY_SIMPLEX,
